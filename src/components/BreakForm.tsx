...
export type BreakFormProps = {
    ...
    action: (prevState: BreakFormState, formData: FormData) => Promise<BreakFormState>;
    cancelRedirectTo: string;
};

export default function BreakForm({ breakData, action, cancelRedirectTo }: BreakFormProps) {
    ...
    return (
        <form className="flex flex-col gap-6" action={formAction}>
            ...
            <div className="flex flex-col gap-2">
                ...
                <DatePicker
                    ...
                    dayDisableRule={(date) => {
                        ...
                        // Disable dates before start date or same day as start date
                        if (startDate && dayjs(date).isSame(dayjs(startDate), 'day')) return true;
            ...
            <div className="flex flex-row gap-2 justify-end">
                <Button variant="outline" onClick={() => redirect(cancelRedirectTo)}>
                    Mégsem
                </Button>
                <Button type="submit">
                    {breakData ? "Módosítás" : "Létrehozás"}
                </Button>
            </div>
        </form>
    );
